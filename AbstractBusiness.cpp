...
AbstractBusiness::AbstractBusiness(void)
{
	SOH = "\x01";
}
...
void AbstractBusiness::SetCounterServer(Counter * counter)
{
	this->counter = counter;
}

BUSI_TYPE AbstractBusiness::ConvertIntToBusiType(int val)
{
	switch (val)
	{
	case 1:
...
void AbstractBusiness::ParseRequest(std::string& request)
{
...
bool AbstractBusiness::FilterRequestField(std::string& key)
{
	if (key == "cssweb_sysNo")
	{
		return true;
...
void AbstractBusiness::BeginLog(std::string& request)
{
...
void AbstractBusiness::RetErrRes(Trade::TradeLog::LogLevel level, std::string& response, std::string code, std::string msg)
{
	//retmsg要过滤=
...
void AbstractBusiness::GenResponse(std::string& response, std::string& errCode, std::string& errMsg)
{
	response = "1";
	response += SOH;
	response += "3";
...
void AbstractBusiness::RetNoRecordRes(std::string& response)
{
	response = "1";
	response += SOH;
	response += "3";
...
void AbstractBusiness::EndLog(std::string& response, Trade::TradeLog& logMsg)
{
	// 得到所有日志相关变量值
...
bool AbstractBusiness::DecryptPassword(std::string algo, std::string key, std::string cipher, std::string plain)
{
